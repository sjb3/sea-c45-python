...
def unique_column_values(rows, column_name):
    ...
    for rows in column_name:
        if (row['ID'] == ID and row['State'] == state and 
            row['Pollster'] == Pollster and row('date')== date ):
       
            return(ID, 'States', 'Pollster', 'dates')
...
def pollster_predictions(poll_rows):
    ...
    for rows in PollDataRow:
        if (row['Pollster'] == Pollster):

            return ('Pollster','pollster_predictions')
